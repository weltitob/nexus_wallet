...
Map<int, String>
REVERSE_OPS = OPS.map((String string, int number) => MapEntry(number, string));
...
Uint8List compile(List<dynamic> chunks) {
  final bufferSize = chunks.fold(0, (acc, chunk) {
    int lenC = chunk.length;

    if (chunk is int && acc is int) {
      return acc + 1;
    }
    if (chunk.length == 1 && asMinimalOP(chunk) != null && acc is int) {
      return acc + 1;
    }
    return acc! + pushData.encodingLength(lenC) + lenC;
  });
...
List<dynamic>? decompile(dynamic buffer) {
  ...
  while (i < buffer.length) {
    ...
    if ((opcode > OPS['OP_0']) && (opcode <= OPS['OP_PUSHDATA4'])) {
      ...
      if (d == null) return throw Exception();
      i += d.size;
