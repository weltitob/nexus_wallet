...
class Input {
  ...
  factory Input.clone(Input input) {
    return Input(
      hash: Uint8List.fromList(input.hash),
      index: input.index,
      script: Uint8List.fromList(input.script),
      sequence: input.sequence,
      value: input.value,
      prevOutScript: Uint8List.fromList(input.prevOutScript),
      pubkeys: input.pubkeys.map((pubkey) => Uint8List.fromList(pubkey)),
      signatures: input.signatures.map((signature) => Uint8List.fromList(signature)),
...
class Output {
  ...
  factory Output.clone(Output output) {
    return Output(
      script: Uint8List.fromList(output.script),
      value: output.value,
      valueBuffer: Uint8List.fromList(output.valueBuffer),
      pubkeys: output.pubkeys.map((pubkey) => Uint8List.fromList(pubkey)),
      signatures: output.signatures.map((signature) => Uint8List.fromList(signature)),
    );
  }
